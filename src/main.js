...
async function onDeviceReady() {
	...
	if (settings.value.checkForAppUpdates && navigator.onLine) {
		cordova.plugin.http.sendRequest(
			"https://api.github.com/repos/ryzix-dev/ryzix-code/releases/latest",
			{
				method: "GET",
				responseType: "json",
			},
			(response) => {
				...
				if (hasUpdate) {
					acode.pushNotification(
						"Update Available",
						`Ryzix Code ${release.tag_name} is now available! Click here to checkout.`,
						{
							icon: "update",
							type: "warning",
							action: () => {
